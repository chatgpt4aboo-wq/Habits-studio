...
Generated from `assets-inbox/models/` by `scripts/prepare_assets.py`.
...
matte would be guesswork — they are presented as photographic plates instead.
The backdrop is read from the border only to locate the figure, and to fill the
frame when it is widened, so there is no seam.
